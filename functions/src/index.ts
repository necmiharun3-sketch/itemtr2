...
export {
  prepareShopierBatchOrders,
  createShopierCheckout,
  shopierWebhook,
} from "./payment/shopierHandlers";
export {
...
export const createPaymentIntent = onCall(async (request) => {
  ...
  const { orderId, amountCents } = (request.data || {}) as {
    orderId?: string;
    amountCents?: number;
  };
  ...
  if (!Number.isInteger(amountCents) || (amountCents as number) <= 0)
    throw new HttpsError(
      "invalid-argument",
      "amountCents must be positive int",
    );
...
export const mockPaymentWebhook = onRequest(async (req: any, res: any) => {
  ...
  const { intentId, status } = (req.body || {}) as {
    intentId?: string;
    status?: string;
  };
...
export const mockCapturePayment = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx: FirebaseFirestore.Transaction) => {
    ...
    const orderId = String(intent.orderId || "");
    if (!orderId)
      throw new HttpsError("failed-precondition", "intent missing orderId");
    ...
    if (order.buyerId !== uid)
      throw new HttpsError("permission-denied", "buyer mismatch");
...
export const confirmDelivery = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx: FirebaseFirestore.Transaction) => {
    ...
    const order = orderSnap.data() as any;
    if (order.buyerId !== uid)
      throw new HttpsError("permission-denied", "not buyer");
    if (order.status === "disputed")
      throw new HttpsError("failed-precondition", "order disputed");
    if (order.escrowStatus !== "held")
      throw new HttpsError("failed-precondition", "escrow not held");

    const sellerId = String(order.sellerId || "");
    const amountCents = Number(
      order.amountCents || Math.round(Number(order.price || 0) * 100),
    );
    if (!sellerId || !Number.isInteger(amountCents) || amountCents <= 0)
...
export const openDispute = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx: FirebaseFirestore.Transaction) => {
    ...
    if (order.status === "completed")
      throw new HttpsError("failed-precondition", "order completed");
...
export const markDelivered = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx: FirebaseFirestore.Transaction) => {
    ...
    if (String(order.sellerId || "") !== uid)
      throw new HttpsError("permission-denied", "not seller");
    if (order.status === "disputed")
      throw new HttpsError("failed-precondition", "order disputed");
...
export const payTradeCashDifference = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx: FirebaseFirestore.Transaction) => {
    ...
    if (offer.senderUserId !== uid)
      throw new HttpsError("permission-denied", "not sender");
    if (offer.status !== "accepted")
      throw new HttpsError("failed-precondition", "offer not accepted");
    if (offer.cashPaid)
      throw new HttpsError("failed-precondition", "cash already paid");
    if (!offer.offeredCashAmount || offer.offeredCashAmount <= 0)
      throw new HttpsError("failed-precondition", "no cash to pay");
...
export const rejectTradeOffer = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx: FirebaseFirestore.Transaction) => {
    ...
    if (offer.status !== "pending" && offer.status !== "viewed")
      throw new HttpsError("failed-precondition", "offer cannot be rejected");
...
export const cancelTradeOffer = onCall(async (request) => {
  ...
  await db.runTransaction(async (tx: FirebaseFirestore.Transaction) => {
    ...
    if (offer.senderUserId !== uid)
      throw new HttpsError("permission-denied", "not sender");
    if (offer.status !== "pending" && offer.status !== "viewed")
      throw new HttpsError("failed-precondition", "offer cannot be cancelled");
...
export const createTradeOffer = onCall(async (request) => {
  ...
  if (!targetListingId)
    throw new HttpsError("invalid-argument", "targetListingId required");
  ...
  await db.runTransaction(async (tx: FirebaseFirestore.Transaction) => {
    ...
    if (!existingSnap.empty)
      throw new HttpsError("already-exists", "active offer exists");
...
export const notifyTradeMessage = onCall(async (request) => {
  ...
  const otherUserId =
    uid === offer.senderUserId ? offer.receiverUserId : offer.senderUserId;
  const userRef = db.collection("users").doc(uid);
...
export const notifyTradeDispute = onCall(async (request) => {
  ...
  await db.collection("notifications").add({
    ...
    message:
      "Bu takas için bir uyuşmazlık kaydı oluşturuldu. Destek ekibi inceleyecektir.",
    isRead: false,
    link: `/trade/offers/${offerId}`,
    createdAt: admin.firestore.FieldValue.serverTimestamp(),
  });
