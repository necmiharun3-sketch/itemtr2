...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Kayıt başarılı! E-posta doğrulaması gerekli.');
      
      setRegisteredUser(user);
      setRegisteredPhone(cleanPhone);

      // Phone verification: trigger smsSend
      try {
        ...
        toast.error('Doğrulama kodu gönderilemedi, lütfen ayarlardan tekrar deneyin.');
        navigate('/', { replace: true, state: { authTransition: true } });
      }
    } catch (error: any) {
  ...
  const handleVerifySms = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await verify({ phone: registeredPhone, code: verificationCode });
  ...
  return (
    <div className="relative -mx-4 sm:-mx-6 lg:-mx-8 -mt-6">
      <SEOHead 
        title={step === 'register' ? "Kayıt Ol - itemTR" : "Telefon Doğrulama - itemTR"}
        ...
        <div className="w-full max-w-md bg-black/55 backdrop-blur rounded-2xl border border-white/10 p-8 shadow-2xl">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-extrabold text-white mb-2">
              {step === 'register' ? 'Kayıt Ol' : 'Telefon Doğrulama'}
            </h1>
            <p className="text-white/65 text-sm">
              {step === 'register' 
                ? 'Aramıza katılın ve avantajlardan yararlanın.' 
                : `${registeredPhone} numaralı telefona gönderilen kodu girin.`}
            </p>
          </div>
          ...
          {step === 'register' ? (
            ...
            <form className="space-y-5" onSubmit={handleVerifySms}>
              <div>
                <label htmlFor="verify-code" className="block text-sm font-medium text-white/75 mb-1.5">Doğrulama Kodu</label>
                <div className="relative">
                  <Smartphone className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-white/40" />
                  <input
                    id="verify-code"
                    ...
                    className="w-full bg-black/30 border border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-white text-center text-2xl tracking-[0.5em] focus:outline-none focus:border-[#ff6a00] transition-colors font-mono"
                  />
                </div>
              </div>
              ...
              <button
                ...
                className="w-full text-center text-sm text-white/45 hover:text-white/75 transition-colors"
              >
                Bilgileri Düzenle
              </button>
            </form>
